...
該研究把棄答選項整個拿掉，結果顯示與臨床無關的身分描述會直接變成決勝因素，正好對照本篇加上棄答層所換到的可靠度: [臨床條件一模一樣時，LLM 先給誰？30,618 次強制二選一的分配稽核](2026-09-22-llm-social-status-resource-allocation.md)

該研究把同一組關注點移到純文字的結構化超音波描述上，同樣以棄答／未明確分類為獨立觀察對象，並與既有的臨床決策規則並列比較: [把超音波描述交給 ChatGPT 判良惡性：300 顆附件腫塊，和 IOTA 規則、ADNEX 與專家差多少？](2026-09-22-chatgpt-adnexal-mass-ultrasound-triage.md)
