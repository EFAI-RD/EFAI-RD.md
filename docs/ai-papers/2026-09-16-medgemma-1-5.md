...
- 作者只明列部分舊基準退步，且指出 token-overlap 類評估本身有品質限制；模型變得更通用，不代表每個任務都更好。[(ref: main limitations)](https://arxiv.org/html/2604.05081v2#S4.SS0.SSS0.Px2)
- 技術報告沒有提供前瞻性臨床驗證、校準、公平性、錯誤嚴重度或工作流程效益證據；因此不能由離線分數推論可直接臨床部署。

## 與書庫其他文章的關係

RADAR 的跨族群評估把 MedGemma1.5 列為未微調的比較對象（in-scope 21 項 AUC 0.555），兩篇可對照原生支援 3D 體積的通用模型與以器官分割為前提的專用對齊，各自的能力邊界與評估分母: [RADAR：把腹部 CT 拆成 18 個器官再對齊報告，通才模型能到專家水準嗎？](2026-09-21-radar-abdominal-ct-generalist.md)

一句話敘述關係: [醫療視覺語言模型：從 CLIP 對齊到有定位報告](../ai-basics/medical-vlm.md) 把本篇放在通才醫療多模態基礎模型分岔；上游是對齊與指令微調，而非從零取代 CLIP。
...
MedGPT-oss 同樣是通用醫療 VLM，但改用開放 20B 語言骨幹與三階段資料配方；兩篇可對照 2D 情境學習與 MedGemma 1.5 的 3D／全切片能力邊界: [MedGPT-oss：20B 模型能靠簡單架構與三階段資料配方追上更大的醫療 VLM 嗎？](2026-09-16-medgpt-oss.md)

ModaLens 用 MedGemma-27B／4B 做成對影像交換稽核，補足技術報告未直接回答的模態依賴問題: [ModaLens：有報告可讀時，醫療 VLM 還會看影像嗎？](2026-09-17-modalens-image-sensitivity.md)

BrainVLM 把多參數腦部 MRI 收斂成專科分類、報告、信心校準與讀者研究，可和 MedGemma 1.5 的通用 3D volume 路線對照專科模型如何補足工作流程證據: [BrainVLM：從多序列 MRI 到前瞻收案，信心分數能讓腦瘤診斷更可靠嗎？](2026-09-17-brainvlm-brain-tumor-diagnosis.md)

胸片結核篩檢稽核把 MedSigLIP 當成受測檢查點之一，顯示同一家族的醫療影像文字編碼器在換世代、換負類與換閾值後結論如何改寫: [稽核胸片結核篩檢的醫療 VLM：換一個評估條件，哪一種結論還站得住？](2026-09-21-cxr-tb-vlm-portability-audit.md)
...
- `main`：[MedGemma 1.5 Technical Report（HTML）](https://arxiv.org/html/2604.05081v2)；本文使用 [§1](https://arxiv.org/html/2604.05081v2#S1)、[§2](https://arxiv.org/html/2604.05081v2#S2)、[Table 1](https://arxiv.org/html/2604.05081v2#S2.T1)、[Table 4](https://arxiv.org/html/2604.05081v2#S3.T4)、[limitations](https://arxiv.org/html/2604.05081v2#S4.SS0.SSS0.Px2) 與 [Appendix A](https://arxiv.org/html/2604.05081v2#A1)。
